...
async def incoming_message_f(client, message):
    ...
    if rep_mess:
        ...
        if not rep_mess.media or str(file_name).lower().endswith(".torrent"):
            ...
            if user_command == BotCommands.LeechCommand.lower():
                await i_m_sefg.edit("<b>No download source provided.</b>")
                return
            is_file = True
            dl_url = rep_mess
    ...
    if dl_url is not None:
        ...
        if user_command == BotCommands.ExtractCommand.lower():
            is_unzip = True
        elif user_command == BotCommands.ArchiveCommand.lower():
            is_zip = True

        if user_command == BotCommands.RcloneLeechCommand.lower():
            is_cloud = True
        if user_command == BotCommands.RcloneLeechExtractCommand.lower():
            is_cloud = True
            is_unzip = True
        elif user_command == BotCommands.RcloneLeechArchiveCommand.lower():
            is_cloud = True
            is_zip = True
...
async def g_yt_playlist(client, message):
    ...
    if message.reply_to_message:
        ...
        if user_command == BotCommands.RclonePlaylistYoutubeDownloaderCommand.lower():
            is_cloud = True
    elif len(message.command) == 2:
        ...
        await message.reply_text("<b>Reply with a YouTube Playlist link.</b>", quote=True)
        return
